...
const base_key_type_1 = require("./base_key_type");
...
class SessionBuilder {
    ...
    async initOutgoing(device) {
        ...
        // Verifica identidade (pode usar fqAddr ou addr.id - use oq fizer sentido;
        // manter consistente com outras partes do código é bom)
        if (!(await this.storage.isTrustedIdentity(fqAddr, device.identityKey))) {
            throw new errors.UntrustedIdentityKeyError(this.addr.id, device.identityKey);
        }
        // Verifica assinatura do signedPreKey (operação CPU)
        ...
        // Inicializa a estrutura de session (que usa storage.getOurIdentity internamente).
        // initSession só lê getOurIdentity e gera dados — deixamos isso fora do lock
    ...
    async initSession(isInitiator, ourEphemeralKey, ourSignedKey, theirIdentityPubKey, theirEphemeralPubKey, theirSignedPubKey, registrationId) {
        ...
        session.indexInfo = {
            ...
            baseKeyType: isInitiator ? base_key_type_1.BaseKeyType.OURS : base_key_type_1.BaseKeyType.THEIRS,
            closed: -1,
        };
        if (isInitiator) {
    ...
    calculateSendingRatchet(session, remoteKey) {
        ...
        session.addChain(ratchet.ephemeralKeyPair.pubKey, {
            ...
            chainType: chain_type_1.ChainType.SENDING,
        });
